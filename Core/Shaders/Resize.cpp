...
Resize::Resize(int srcWidth, int srcHeight, int destWidth, int destHeight)
    : imageTexture(destWidth, destHeight, GL_RGBA, GL_RGB, GL_UNSIGNED_BYTE, false),
      vertexTexture(destWidth, destHeight, GL_RGBA32F, GL_LUMINANCE, GL_FLOAT, false),
      timeTexture(
          destWidth,
          destHeight,
...
void Resize::image(GPUTexture* source, Img<Eigen::Matrix<uint8_t, 3, 1>>& dest) {
  ...
  glReadPixels(
      0, 0, imageRenderBuffer.width, imageRenderBuffer.height, GL_RGB, GL_UNSIGNED_BYTE, dest.data);
...
void Resize::vertex(GPUTexture* source, Img<Eigen::Vector4f>& dest) {
  ...
  glReadPixels(
      0, 0, vertexRenderBuffer.width, vertexRenderBuffer.height, GL_RGBA, GL_FLOAT, dest.data);
...
void Resize::time(GPUTexture* source, Img<uint16_t>& dest) {
  ...
  glReadPixels(
      0,
      0,
      timeRenderBuffer.width,
      timeRenderBuffer.height,
      GL_LUMINANCE_INTEGER_EXT,
      GL_UNSIGNED_SHORT,
      dest.data);
