...
void RGBDOdometry::initICP(GPUTexture* predictedVertices, GPUTexture* predictedNormals) {
  ...
  copyMaps(vmapPtr, nmapPtr, width, height, vmaps_tmp, vmaps_curr_[0], nmaps_curr_[0]);
...
void RGBDOdometry::initICPModel(
  ...
  copyMaps(vmapPtr, nmapPtr, width, height, vmaps_tmp, vmaps_g_prev_[0], nmaps_g_prev_[0]);
  ...
  Eigen::Matrix<float, 3, 3, Eigen::RowMajor> R_wc = T_wc.rotationMatrix().cast<float>();
  Eigen::Vector3f t_wc = T_wc.translation().cast<float>();

  mat33 device_R_wc = R_wc;
  float3 device_t_wc = *reinterpret_cast<float3*>(t_wc.data());
...
void RGBDOdometry::populateRGBDData(
    GPUTexture* rgb,
...
void RGBDOdometry::getIncrementalTransformation(
  ...
  Eigen::Matrix<float, 3, 3, Eigen::RowMajor> Rprev = T_wc.rotationMatrix().cast<float>();
  Eigen::Vector3f tprev = T_wc.translation().cast<float>();
  ...
  if (so3) {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      Eigen::Matrix<double, 3, 3, Eigen::RowMajor> rotUpdate =
          OdometryProvider::rodrigues(delta.cast<double>());
  ...
  Eigen::JacobiSVD<Eigen::Matrix3d> svd(
      Rcurr.cast<double>(), Eigen::ComputeFullU | Eigen::ComputeFullV);

  T_wc.translation() = tcurr.cast<double>();
  T_wc.setRotationMatrix(svd.matrixU() * svd.matrixV().transpose());
}

void RGBDOdometry::getIncrementalTransformationNew(
